...
public class TarotCardsPanel extends JPanel {
    ...
    public TarotCardsPanel(JButton drawButton) {
        ...
        try {
            backCard = ImageIO.read(
                    Objects.requireNonNull(getClass().getResource("/TarotCards/card_back.png"))
            );
        } catch (IOException e) {
        ...
        JLabel Card1 = createCardNameLabel();
        add(Card1, gbc);

        gbc.gridx = 1;

        JLabel Card2 = createCardNameLabel();
        add(Card2, gbc);

        gbc.gridx = 2;

        JLabel Card3 = createCardNameLabel();
        add(Card3, gbc);

        // Third row — card meanings
        gbc.gridy = 2;
    ...
    /**
     * Creates a consistently styled JLabel for card names.
     */
    private JLabel createCardNameLabel() {
        JLabel label = new JLabel("");
        label.setFont(AppFonts.regular(16f));
    ...
    private JTextArea createMeaningArea() {
        ...
        area.setOpaque(true);
        area.setBackground(TAROT_BACKGROUND);
